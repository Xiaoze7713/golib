...
func RootPath() string {
	if envRootPath == "" {
		if os.Getenv("WS_ROOT_PATH") != "" {
			SetRootPath(os.Getenv("WS_ROOT_PATH"))
		} else {
...
// SetHttpPort 设置http端口
func SetHttpPort(port string) {
	envHttpPort = port
	log.Printf("[env] SetHttpPort= %s\n", envHttpPort)
}

func HttpPort() string {
	if envHttpPort == "" {
		SetHttpPort(_defaultHttpPort)
	}
	return envHttpPort
}
